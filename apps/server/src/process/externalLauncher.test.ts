...
const launchCustomEditor = (input: {
  readonly customEditors: ReadonlyArray<{
    readonly id: string;
  ...
  Effect.gen(function* () {
    const fileSystem = yield* FileSystem.FileSystem;
    const path = yield* Path.Path;
    const binDir = yield* fileSystem.makeTempDirectoryScoped({ prefix: "t3-custom-editors-" });
    for (const editor of input.customEditors) {
      const command = editor.command[0];
      yield* fileSystem.writeFileString(path.join(binDir, command), "#!/bin/sh\nexit 0\n");
      yield* fileSystem.chmod(path.join(binDir, command), 0o755);
    }
    ...
    yield* Effect.gen(function* () {
      const launcher = yield* ExternalLauncher.ExternalLauncher;
      yield* launcher.launchEditor(
        { editor: input.editor as never, cwd: input.cwd },
        input.customEditors as never,
      );
    }).pipe(
      Effect.provide(
        testLayer({
...
it.effect("resolves custom editors with {path} placeholder substitution", () =>
  Effect.gen(function* () {
    const spawned = yield* launchCustomEditor({
      customEditors: [
        {
          id: "nvim-ghostty",
          name: "Neovim (Ghostty)",
          command: ["ghostty", "-e", "nvim", "{path}"],
        },
      ],
      editor: "custom:nvim-ghostty",
      cwd: "/tmp/workspace",
    });
    assert.equal(spawned.command, "ghostty");
    assert.deepEqual(spawned.args, ["-e", "nvim", "/tmp/workspace"]);
    assert.equal(spawned.options.detached, true);
  }),
);
...
it.effect("appends the target path when a custom editor command has no placeholder", () =>
  Effect.gen(function* () {
    const spawned = yield* launchCustomEditor({
      customEditors: [{ id: "nvim", name: "Neovim", command: ["nvim"] }],
      editor: "custom:nvim",
      cwd: "/tmp/workspace",
    });
    assert.equal(spawned.command, "nvim");
      ...
      customEditors: [
        {
          id: "wezterm-nvim",
          name: "Neovim (WezTerm)",
          command: ["wezterm", "start", "--cwd={path}", "nvim", "{path}"],
        },
      ],
      editor: "custom:wezterm-nvim",
      cwd: "/tmp/workspace",
    });
    assert.equal(spawned.command, "wezterm");
    assert.deepEqual(spawned.args, ["start", "--cwd=/tmp/workspace", "nvim", "/tmp/workspace"]);
  }),
);
...
it.effect("fails for custom editor ids without a matching definition", () =>
  Effect.gen(function* () {
    const launcher = yield* ExternalLauncher.ExternalLauncher;
    const error = yield* launcher
      .launchEditor({ editor: "custom:missing" as never, cwd: "/tmp/workspace" }, [])
      .pipe(Effect.flip);
    assert.instanceOf(error, ExternalLauncher.ExternalLauncherUnknownEditorError);
    assert.equal(error.editor, "custom:missing");
  }).pipe(Effect.provide(testLayer({ platform: "linux", env: { PATH: "" } }))),
);
